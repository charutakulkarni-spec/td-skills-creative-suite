...
2. **No guidelines yet** — delegate to the `brand-onboarding` skill: "Let's set up your brand guidelines first. I'll walk you through it." Follow `creative-skills/brand-onboarding/SKILL.md`.
...
**If yes — training knowledge:** Analyze across three dimensions using your knowledge:
...
Synthesize findings into a brief summary with 3-5 actionable recommendations. Keep it concise — this informs ideation, not a standalone deliverable.

**If no:** Skip directly to creative direction.

**Checkpoint after trend analysis:** After presenting trends, explain how they'll be used and wait for the user before proceeding:

> "These trends will inform the creative directions I brainstorm next — I'll prioritize ideas that align with what's working in your space. Take a moment to review, and let me know when you're ready to move on to creative direction."

Do NOT proceed to 3b until the user responds.

### 3b: Creative Direction

Delegate to the `multi-channel-ad-ideation` skill's creative direction phase. Read and follow `creative-skills/multi-channel-ad-ideation/SKILL.md` — specifically the "Creative Direction Ideation Phase" section.
...
Generate 3-5 strategic directions in a comparison table with 7-dimension quality scoring. Let the user select, iterate, combine, or request more.

**When done:** Confirm selected direction(s). Then **you MUST ask about the mood board** before moving to Phase 4 — do NOT skip this checkpoint.
...
Present recommended assets with standard dimensions:
...
Only show tables for the user's selected channels — don't dump all channels.

### Confirm selections

Ask the user to confirm, remove, or add assets.

### Attribute-based variations

After confirming the asset list, ask if they want to personalize assets by an audience attribute:
...
**If yes:**
1. Present the supported attributes (see table below)
2. Ask which attribute to vary on
3. Ask for the attribute values — suggest common values, let user customize
4. **Variations count = number of attribute values**
...
| Region | Localized references (currency, landmarks, cultural cues) | US, APAC, EMEA, LATAM |
...
|---------|-------|-----------|-----------|--------|------------|
| Meta | Feed Post | 1080x1080 | Gender | Male, Female | 2 |

**Example — Language:**

| Channel | Asset | Dimensions | Attribute | Values | Variations |
|---------|-------|-----------|-----------|--------|------------|
| Meta | Feed Post | 1080x1080 | Language | EN, HI, JA, ES | 4 |

**If no:** Variations = 1, no attribute column needed.

Users can set different attributes per asset or apply one attribute across all assets.

---

## Phase 5: Review & Output

Two outputs: a workspace note (persistence) and a grid dashboard (visual).

### 5a: Workspace Note

Save the complete brief as a note in the active workspace:
...
Render a visual overview via `preview_grid_dashboard`. Write the YAML file incrementally (see `treasure-work-skills/grid-dashboard/SKILL.md` for build rules).
...
Build incrementally: write rows 1-2 first, then edit to append rows 3-4, then call `preview_grid_dashboard` once.
